...
- Preserve unrelated user changes. If unexpected changes, conflicts, untracked files, detached HEAD, or an unexpected branch are found, stop before staging or committing and report them. A Task Spec may document a pre-existing excluded baseline change; the narrowly defined automatic Orca `package-lock.json` baseline is governed only by [`docs/codex-operating-policy.md`](docs/codex-operating-policy.md). Leave every excluded baseline untouched and unstaged, and reverify its recorded evidence before checkpoint commit.
